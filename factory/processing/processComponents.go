...
import (
	...
	"github.com/kalyan3104/k-chain-go/dataRetriever/factory/containers"
	"github.com/kalyan3104/k-chain-go/dataRetriever/factory/epochProviders"
	requesterscontainer "github.com/kalyan3104/k-chain-go/dataRetriever/factory/requestersContainer"
	"github.com/kalyan3104/k-chain-go/dataRetriever/factory/resolverscontainer"
	disabledResolversContainer "github.com/kalyan3104/k-chain-go/dataRetriever/factory/resolverscontainer/disabled"
	storagerequesterscontainer "github.com/kalyan3104/k-chain-go/dataRetriever/factory/storageRequestersContainer"
	"github.com/kalyan3104/k-chain-go/dataRetriever/requestHandlers"
	...
	"github.com/kalyan3104/k-chain-go/epochStart/shardchain"
	errorsk "github.com/kalyan3104/k-chain-go/errors"
	"github.com/kalyan3104/k-chain-go/factory"
	mainFactory "github.com/kalyan3104/k-chain-go/factory"
	"github.com/kalyan3104/k-chain-go/factory/disabled"
	"github.com/kalyan3104/k-chain-go/fallback"
	"github.com/kalyan3104/k-chain-go/genesis"
	"github.com/kalyan3104/k-chain-go/genesis/checking"
	processGenesis "github.com/kalyan3104/k-chain-go/genesis/process"
	"github.com/kalyan3104/k-chain-go/p2p"
	"github.com/kalyan3104/k-chain-go/process"
	"github.com/kalyan3104/k-chain-go/process/block"
	"github.com/kalyan3104/k-chain-go/process/block/bootstrapStorage"
	"github.com/kalyan3104/k-chain-go/process/block/cutoff"
	"github.com/kalyan3104/k-chain-go/process/block/pendingMb"
	"github.com/kalyan3104/k-chain-go/process/block/poolsCleaner"
	"github.com/kalyan3104/k-chain-go/process/block/preprocess"
	"github.com/kalyan3104/k-chain-go/process/block/processedMb"
	"github.com/kalyan3104/k-chain-go/process/factory/interceptorscontainer"
	"github.com/kalyan3104/k-chain-go/process/headerCheck"
	"github.com/kalyan3104/k-chain-go/process/heartbeat/validator"
	"github.com/kalyan3104/k-chain-go/process/peer"
	"github.com/kalyan3104/k-chain-go/process/receipts"
	"github.com/kalyan3104/k-chain-go/process/smartContract"
	"github.com/kalyan3104/k-chain-go/process/sync"
	"github.com/kalyan3104/k-chain-go/process/track"
	"github.com/kalyan3104/k-chain-go/process/transactionLog"
	"github.com/kalyan3104/k-chain-go/process/txsSender"
	"github.com/kalyan3104/k-chain-go/redundancy"
	"github.com/kalyan3104/k-chain-go/sharding"
	"github.com/kalyan3104/k-chain-go/sharding/networksharding"
	...
	updateDisabled "github.com/kalyan3104/k-chain-go/update/disabled"
	updateFactory "github.com/kalyan3104/k-chain-go/update/factory"
	"github.com/kalyan3104/k-chain-go/update/trigger"
...
func (pcf *processComponentsFactory) newValidatorStatisticsProcessor() (process.ValidatorStatisticsProcessor, error) {
	...
	if check.IfNil(genesisHeader) {
		return nil, errorsk.ErrGenesisBlockNotInitialized
	}
...
func checkProcessComponentsArgs(args ProcessComponentsFactoryArgs) error {
	...
	if check.IfNil(args.AccountsParser) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilAccountsParser)
	}
	if check.IfNil(args.GasSchedule) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilGasSchedule)
	}
	if check.IfNil(args.Data) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilDataComponentsHolder)
	}
	if check.IfNil(args.Data.Blockchain()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilBlockChainHandler)
	}
	if check.IfNil(args.Data.Datapool()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilDataPoolsHolder)
	}
	if check.IfNil(args.Data.StorageService()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilStorageService)
	}
	if check.IfNil(args.CoreData) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilCoreComponentsHolder)
	}
	if check.IfNil(args.CoreData.EconomicsData()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilEconomicsData)
	}
	if check.IfNil(args.CoreData.GenesisNodesSetup()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilGenesisNodesSetupHandler)
	}
	if check.IfNil(args.CoreData.AddressPubKeyConverter()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilAddressPublicKeyConverter)
	}
	if check.IfNil(args.CoreData.EpochNotifier()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilEpochNotifier)
	}
	if check.IfNil(args.CoreData.ValidatorPubKeyConverter()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilPubKeyConverter)
	}
	if check.IfNil(args.CoreData.InternalMarshalizer()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilInternalMarshalizer)
	}
	if check.IfNil(args.CoreData.Uint64ByteSliceConverter()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilUint64ByteSliceConverter)
	}
	if check.IfNil(args.Crypto) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilCryptoComponentsHolder)
	}
	if check.IfNil(args.Crypto.BlockSignKeyGen()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilBlockSignKeyGen)
	}
	if check.IfNil(args.State) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilStateComponentsHolder)
	}
	if check.IfNil(args.State.AccountsAdapter()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilAccountsAdapter)
	}
	if check.IfNil(args.Network) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilNetworkComponentsHolder)
	}
	if check.IfNil(args.Network.NetworkMessenger()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilMessenger)
	}
	if check.IfNil(args.Network.InputAntiFloodHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilInputAntiFloodHandler)
	}
	if args.SystemSCConfig == nil {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilSystemSCConfig)
	}
	if check.IfNil(args.BootstrapComponents) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilBootstrapComponentsHolder)
	}
	if check.IfNil(args.BootstrapComponents.ShardCoordinator()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilShardCoordinator)
	}
	if check.IfNil(args.BootstrapComponents.EpochBootstrapParams()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilBootstrapParamsHandler)
	}
	if check.IfNil(args.StatusComponents) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilStatusComponentsHolder)
	}
	if check.IfNil(args.StatusComponents.OutportHandler()) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilOutportHandler)
	}
	if check.IfNil(args.HistoryRepo) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilHistoryRepository)
	}
	if check.IfNil(args.StatusCoreComponents) {
		return fmt.Errorf("%s: %w", baseErrMessage, errorsk.ErrNilStatusCoreComponents)
	}
