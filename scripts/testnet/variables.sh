...
# Path to k-chain-go. Determined automatically. Do not change.
export KALYANDIR=$(dirname $(dirname $KALYANTESTNETSCRIPTSDIR))

# Enable the Kalyan Proxy. Note that this is a private repository
# (k-chain-proxy-go).
export USE_PROXY=1

# Enable the Kalyan Transaction Generator. Note that this is a private
# repository (k-chain-txgen-go).
export USE_TXGEN=0
...
# anyway.
export TESTNETDIR="$HOME/Kalyan/testnet"

# Path to k-chain-deploy-go, branch: master. Default: near k-chain-go.
export CONFIGGENERATORDIR="$(dirname $KALYANDIR)/k-chain-deploy-go/cmd/filegen"
...
# Path to k-chain-proxy-go, branch: master. Default: near k-chain-go.
export PROXYDIR="$(dirname $KALYANDIR)/k-chain-proxy-go/cmd/proxy"
...
# Path to k-chain-txgen-go. Default: near k-chain-go.
export TXGENDIR="$(dirname $KALYANDIR)/k-chain-txgen-go/cmd/txgen"
...
LOCAL_OVERRIDES="$KALYANTESTNETSCRIPTSDIR/local.sh"
if [ -f "$LOCAL_OVERRIDES" ]; then
  source "$KALYANTESTNETSCRIPTSDIR/local.sh"
fi
