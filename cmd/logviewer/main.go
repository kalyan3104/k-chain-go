...
`
	...
	address = cli.StringFlag{
		Name:        "address",
		Usage:       "Address and port number on which the application will try to connect to the k-chain-go node",
...
func initCliFlags() {
	...
	cliApp.Name = "Kalyan Logviewer App"
	cliApp.Version = fmt.Sprintf("%s/%s/%s-%s", "1.0.0", runtime.Version(), runtime.GOOS, runtime.GOARCH)
	cliApp.Usage = "Logviewer application used to communicate with k-chain-go node to log the message lines"
...
func startLogViewer(ctx *cli.Context) error {
	...
	// set this log's level to the lowest desired log level that matches received logs from k-chain-go
